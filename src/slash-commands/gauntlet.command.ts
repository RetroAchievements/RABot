...
const gauntletSlashCommand: SlashCommand = {
  ...
  async execute(interaction, _client) {
    ...
    const type: "icon" | "collage" = interaction.options.getString("type", true) as
      | "icon"
      | "collage";
    ...
    const modal = new ModalBuilder()
      ...
      .addTextDisplayComponents((text) =>
        text.setContent(
          "Use this form to add all the information about your icons.\n\n" +
            (type == "icon"
      ...
      .addLabelComponents(
        (originalLabel) =>
          originalLabel
            ...
            .setDescription(
              type == "icon"
                ? "Leave blank to use current icon"
                : "Upload a preview of what the icons currently look like",
            )
        ...
        (contendersLabel) =>
          contendersLabel
            ...
            .setFileUploadComponent((file) =>
              file.setCustomId("icon:contenders").setMaxValues(10).setRequired(true),
            ),
        ...
        (contactDevLabel) =>
          multipleAuthors
            ? contactDevLabel
                ...
                .setDescription(
                  "You must contact every active developer, unless an exemption applies. Select all relevant options.",
                )
                .setCheckboxGroupComponent((radioGroup) =>
                  radioGroup
            ...
            : contactDevLabel
                ...
                .setDescription(
                  "This is required unless you have one of the exemptions. It will be added to your message for you.",
                )
        ...
        (additionalInfoLabel) =>
          additionalInfoLabel
            ...
            .setDescription(
              "Anything else you want to add? You can also leave a message below this poll for more control.",
            )
...
export class GauntletCommand {
  ...
  async buildIconGauntletComponent(event: ModalSubmitInteraction) {
    ...
    const originalIconUrl =
      originalIcon == null || originalIcon.size == 0
        ? "https://media.retroachievements.org" + info.imageIcon
        : originalIcon.at(0)!.url;
    ...
    for (let i = 0; i < contenders.size; i++) {
      ...
      contenderComponents.push(
        new SectionBuilder()
          .setThumbnailAccessory((contenderThumbnail) => contenderThumbnail.setURL(contender.url))
          .addTextDisplayComponents((contenderText) =>
            contenderText.setContent("# 1️⃣ Contender " + (i + 1)),
          ),
      );
    }
    ...
    const components = [
      new TextDisplayBuilder().setContent(
        `Attention @icon-gauntlet! A new gauntlet has been started by <@!${author.id}>.`,
      ),
      new ContainerBuilder()
        .addTextDisplayComponents(
          (componentLink) =>
            componentLink.setContent(
              `Game/Hub: [${info.title}](https://retroachievements.org/game/${info.id})`,
            ),
  ...
  async buildCollageGauntletComponent(event: ModalSubmitInteraction) {
    ...
    let pollContainer = new ContainerBuilder()
      ...
      .addTextDisplayComponents(
        (componentLink) =>
          componentLink.setContent(
            `Game: [${info.title}](https://retroachievements.org/game/${info.id})`,
          ),
      ...
      .addMediaGalleryComponents((mediaGallery) =>
        mediaGallery.addItems((contenderImage) => contenderImage.setURL(originalIcon.at(0)!.url)),
      );
    ...
    for (let i = 0; i < contenders.size; i++) {
      ...
      pollContainer = pollContainer
        ...
        .addMediaGalleryComponents((mediaGallery) =>
          mediaGallery.addItems((contenderImage) => contenderImage.setURL(contender.url)),
        );
    }
